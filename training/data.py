...
def get_nyu_data(batch_size, nyu_data_zipfile='d:/dane/depth/nyu_data.zip '):
    data = extract_zip(nyu_data_zipfile)
    ...
    # Open a file: file
    file = open('d:\\data\\nyu2_train.csv', mode='r')
    nyu2_train_text = file.read()
    file.close()

    file = open('d:\\data\\nyu2_test.csv', mode='r')
    nyu2_test_text = file.read()
    file.close()

    nyu2_train = list((row.split(',') for row in (nyu2_train_text).split('\n') if len(row) > 0))
    nyu2_test = list((row.split(',') for row in (nyu2_test_text).split('\n') if len(row) > 0))
...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            x = np.clip(np.asarray(Image.open(self.file_path +sample[0] )).reshape(480,640,3)/255,0,1)
            y = np.clip(np.asarray(Image.open(self.file_path +sample[1] )).reshape(480,640,1)/255*self.maxDepth,0,self.maxDepth)
...
class NYU_BasicRGBSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):            
            ...
            x = np.clip(np.asarray(Image.open(self.file_path + sample[0])).reshape(480,640,3)/255,0,1)
            y = np.asarray(Image.open(self.file_path + sample[1]), dtype=np.float32).reshape(480,640,1).copy().astype(float) / 10.0
