...
#ten model jest OK!
def create_model_v3(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        base_model_output_shape = base_model.get_layer("pool4_relu").output.shape
        ...
        # Decoder Layers
        decoder = Conv2D(filters=decode_filters / 4, kernel_size=1, padding='same', input_shape=base_model_output_shape, name='conv2')(base_model.get_layer("pool4_relu").output)
        decoder = BilinearUpSampling2D((2, 2), name="upup")(decoder)
        """
